    ...
    public Game () {
        ...
        for (int i = 0; i < WIDTH / SCL; i++) {
            for (int j = 0; j < HEIGHT / SCL; j++) {
                handler.addObject(new Cover(i * SCL, j * SCL, ID.Cover, index++, handler));
            }
        }

        for (int i = 0; i < 25; i++) handler.addObject(new Bomb(0, 0, ID.Bomb, handler));

        for (GameObject obj : handler.object) if (obj.id == ID.Cover) obj.countBombs();
    }
